...
export const StudentPageProfile = (props: StudentQuery) => {
  return <div className="max-md:px-8">
    <div className="relative flex  flex-col md:flex-row md:divide-x md:divide-zinc-800 max-md:border-x border-zinc-800">
      ...
      <div className="flex-grow ">
        ...
        <div className="relative w-full mx-auto max-w-screen-md p-6   grid sm:grid-cols-2 gap-4">
          ...
          <div className=" flex-grow">
            <p data-tina-field={tinaField(props.student, 'hometown')} className='text-zinc-300 text-xl'>{props.student?.hometown}</p>
          </div>
        </div>
        ...
        <div className="relative w-full mx-auto max-w-screen-md p-6  grid sm:grid-cols-2 gap-4">
          ...
          <div className=" flex-grow">
            <p data-tina-field={tinaField(props.student, 'phoneNumber')} className='text-zinc-300 text-xl'>{props.student?.phoneNumber}</p>
          </div>
        </div>
        ...
        <div className="relative w-full mx-auto max-w-screen-md p-6  grid sm:grid-cols-2 gap-4">
          ...
          <div className=" flex-grow">
            <p data-tina-field={tinaField(props.student, 'email')} className='text-zinc-300 text-xl'>{props.student?.email}</p>
          </div>
        </div>
            ...
            <ul className="flex flex-wrap gap-2">
              {props.student?.skills.map(item => {
                return <li key={item?.label}>
                  <h6 data-tina-field={item && tinaField(item)}><span className='block px-4 border border-zinc-800 rounded-full bg-white bg-opacity-10 backdrop-blur-md leading-none py-1.5 pb-2 text-zinc-300 text-base'>{item?.label}</span></h6>
                </li>
              })}
            </ul>
          </div>
          ...
          <div className=" flex-grow">
            <ul className="flex flex-wrap gap-2">
              {props.student?.practicalExperiences.map(item => {
                return <li key={item?.label}>
                  <h6 data-tina-field={item && tinaField(item)} className="text-zinc-300 text-xl ">{item?.label}</h6>
                </li>
              })}
            </ul>
          </div>
